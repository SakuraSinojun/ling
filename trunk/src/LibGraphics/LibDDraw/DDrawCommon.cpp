...
bool com_attachSurface(LPDIRECTDRAWSURFACE7 lpddSurDest, LPDIRECTDRAWSURFACE7 lpddSurSrc, LPRECT lprcDest, LPRECT lprcSrc, bool bTransparent)
{
        if(bTransparent)
        {
                if (FAILED (lpddSurDest->Blt (lprcDest, lpddSurSrc, lprcSrc, DDBLT_WAIT | DDBLT_KEYSRC, NULL)))
                {
                        return false;
                }
        }
        ...
        {
                if (FAILED (lpddSurDest->Blt (lprcDest, lpddSurSrc, lprcSrc, DDBLT_WAIT , NULL)))
                {
                        return false;
                }
        }
...
bool com_colorRect(LPDIRECTDRAWSURFACE7 lpds, unsigned long color, LPRECT pRectDest)
{
...
unsigned char *com_lockSurface(LPDIRECTDRAWSURFACE7 lpds, int &lPitch)
{
        DDSURFACEDESC2 ddsd;
	InitStruct(ddsd);

	if (FAILED (lpds->Lock(NULL, &ddsd, DDLOCK_SURFACEMEMORYPTR | DDLOCK_WAIT, NULL)))
	{
		return NULL;
	}
