...
void CMapEditDlg::InitList()
{
        ...
        CDC *dc = this->GetDC();
        HBITMAP hBitmap = CreateCompatibleBitmap(dc->m_hDC, 64, 64);
        m_dcDef.SelectObject(hBitmap);
        DeleteObject(hBitmap);
        ReleaseDC(dc);

        m_dcDef.FillSolidRect(0, 0, 64, 64, RGB(255, 255, 255));
...
void CMapEditDlg::UpdateIconList()
{
        ...
        for(i=0; i<m_curListEnd; i++)
        {
                ...
                str.Format("%s ID: %d", (LPTSTR)(LPCTSTR)m_pInfo[index].name, m_pInfo[index].index);
                
                m_stDis[i].SetWindowText((LPTSTR)(LPCTSTR)str);
        }
...
void CMapEditDlg::CreateNewMap(CString strName, int width, int height, int cellSize)
{
        ...
        memset((char*)m_pMap, 0, sizeof(DWORD)*width*height);
...
void CMapEditDlg::OnLButtonDown(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
        if(PtInRect(&m_rcView, point))
        {
                ...
                {
                        // 0索引具有擦除权
                        // 当选中0索引的图标时，鼠标点击地图上元素的第一个位置
                        // 编辑器就会用0索引来擦除这个元素的全部部分

                        int index = point.x + point.y*m_mapWidth;
                        if(m_pMap[index] !=0 && m_pMap[index] != MAP_MASK)
                        {
                                CellInfo *curInfo = NULL;
                                for(i=0; i<m_bmpNum; i++)
                                {
                                        if(m_pInfo[i].index == m_pMap[index])
                                        {
                                                curInfo = &m_pInfo[i];
                                        }
                                }
                                ...
                                if(NULL == curInfo)
                                {
                                        MessageBox("无法找到地图该元素索引");
                                        return;
                                }

                                for(i=point.x; i<point.x + curInfo->width / m_mapCellSize; i++)
                                {
                                        for(j=point.y; j<point.y + curInfo->height/m_mapCellSize; j++)
                                        {
                                                m_dcView.BitBlt(i * m_mapCellSize, j * m_mapCellSize, 
                                                        m_mapCellSize, m_mapCellSize, &m_dcChoose, 0, 0, SRCCOPY);

                                                m_pMap[i + j*m_mapWidth] = 0;
                                        }
                                }
                        }
                ...
                for(i=point.x ;i<point.x + m_curWidth/m_mapCellSize; i++)
                {
                        for(j=point.y; j<point.y + m_curHeight/m_mapCellSize; j++)
                        {
                                if(m_pMap[i + j*m_mapWidth] != 0 || m_pMap[i+ j*m_mapWidth] == MAP_MASK)
                                {
                                                return;
                                }
                        }
                ...
                for(i=point.x  ; i<point.x + m_curWidth/m_mapCellSize; i++)
                {
                        for(j=point.y; j<point.y + m_curHeight/m_mapCellSize; j++)
                        {
                                m_pMap[i + j*m_mapWidth] = MAP_MASK;
                        }
                }
...
void CMapEditDlg::LoadRes()
{
       ...
       CellInfo *zeroInfo = NULL;
       for(i=0; i<m_bmpNum; i++)
       {
                if(m_pInfo[i].index == 0)
                {
                        zeroInfo = &m_pInfo[i];
                        break;
                }
       ...
       if(i == m_bmpNum)
       {
               MessageBox("缺少索引为0的图标资源，请重新加载");
       }
       else
       {
               if(zeroInfo->width != m_mapCellSize || zeroInfo->height != m_mapCellSize)
               {
                        MessageBox("0索引的图标资源尺寸必须与地图单位尺寸相同，请重新加载");
               }

               for(i=0; i<m_mapWidth; i+=zeroInfo->width/m_mapCellSize)
               {
                       for(j=0; j<m_mapHeight; j+=zeroInfo->height/m_mapCellSize)
                       {
                               m_dcView.BitBlt(i*m_mapCellSize, j*m_mapCellSize, zeroInfo->width, zeroInfo->height,
                                       zeroInfo->dc, 0, 0, SRCCOPY);
                       }
               }
       }
...
void CMapEditDlg::OnLoadMap() 
{
        ...
        if(IDOK == dlg.DoModal())
        {
                ...
                this->CreateNewMap(dlg.GetFileName(), m_map.Width(), m_map.Height(),m_map.Size());


                if(!m_map.GetMapData(m_pMap))
                {
                        MessageBox("获取地图文件信息失败");
                        return;
                }
                ...
                int mapsize = m_mapWidth*m_mapHeight;
                ...
                for(i=0; i<m_mapHeight; i++)
                {
                        ...
                        {
                                if(m_pMap[j + m_mapWidth*i] == MAP_MASK)
                                {
                                        continue;
                                }
                                for(k=0; k<m_bmpNum; k++)
                                {
                                        if(m_pMap[j + m_mapHeight*i] == m_pInfo[k].index)
                                        {
                                                m_dcView.BitBlt(j*m_mapCellSize , i*m_mapCellSize, m_pInfo[k].width, m_pInfo[k].height,
                                                        m_pInfo[k].dc, 0, 0, SRCCOPY);
                                                break;
                                ...
                                if(k == m_bmpNum)
                                {
                                        ...
                                        msg.Format("图标列表没有ID[%d]的图片", m_pMap[j+m_mapHeight*i]);
...
void CMapEditDlg::OnSaveMap() 
{
        ...
        if(IDOK == dlg.DoModal())
        {
                ...
                //SEGMENTHEAD head;
                //if(!SaveMyBmp(m_pInfo[1].hBitmap, head, buffer))
                //{
                ///        MessageBox("保存文件失败");
                //}

                buffer = new char[23094];

                ifstream ifs;
                ifs.open("test.map", ios::out| ios::binary);
                ifs.read((char*)buffer, 23094);
                ifs.close();
                ...
                */
                ...
                DWORD *temp = new DWORD[m_mapWidth * m_mapHeight];

                memset((char*)temp, 0, m_mapWidth * m_mapHeight * sizeof(DWORD));
